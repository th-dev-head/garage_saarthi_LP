...
const appointmentHeroBlocks = [
  {
    label: "CUSTOMER & VEHICLE",
    title: "Vikram Malhotra • GJ-01-AB-4492",
    sub: "Hyundai Creta (Diesel) • +91 98250 *****",
  },
  {
    label: "TIME SLOT & SERVICE TYPE",
    title: "10:30 AM - 11:30 AM • Periodic Maintenance",
    titleClass: "text-xs text-slate-200 block",
  },
  {
    label: "DOORSTEP PICKUP & DRIVER",
    title: "SG Highway, Ahmedabad • Ramesh (Assigned)",
    titleClass: "text-xs font-bold text-amber-400",
  },
  {
    label: "LIVE STATUS",
    title: "Driver Dispatched (Google Maps Nav Active)",
    titleClass: "text-xs font-bold text-emerald-400",
    isFlex: true,
  },
];
...
export default function AppointmentsHero() {
  const handleTrialClick = () => {
    trackEvent("appointment_feature_trial_cta_click", {
      page_path: "/features/appointments/",
      cta_location: "hero",
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
          ...
          <div className="space-y-6">
            <span className="text-xs uppercase tracking-widest bg-orange-600/30 text-orange-400 px-3 py-1.5 rounded-full border border-orange-500/20 font-semibold mb-2 inline-block">
              Workshop Appointment & Pickup-Drop Software
            </span>
            ...
            <p className="text-slate-300 text-sm md:text-base max-w-xl leading-relaxed">
              Schedule service slots in advance, dispatch drivers with live Google Maps navigation, send automated WhatsApp confirmations, and convert appointments to job cards in 1-click.
            </p>
          ...
          <div className="flex justify-center md:justify-end w-full">
            <div className="bg-white/10 backdrop-blur-md border border-white/10 rounded-2xl p-6 shadow-2xl w-full max-w-[340px] lg:max-w-md">
              <div className="flex justify-between items-center pb-4 mb-4 border-b border-white/10">
                <span className="text-xs font-bold text-white">Daily Booking #APT-4092</span>
                <span className="text-[10px] text-green-400 bg-green-500/10 px-2 py-0.5 rounded font-semibold">
                  Pickup Confirmed
                </span>
              </div>
              <div className="space-y-4">
                {appointmentHeroBlocks.map((block, idx) => (
                  <div
                    key={idx}
                    className={`bg-white/5 p-4 rounded-xl border border-white/5 ${
                      block.isFlex ? "flex justify-between items-center" : "space-y-1"
                    }`}
                  >
                    <div>
                      <span className="text-[10px] text-slate-400 block mb-0.5">{block.label}</span>
                      <span className={block.titleClass || "text-sm font-bold text-white"}>
                        {block.title}
                      </span>
                    </div>
                    {block.sub && (
                      <span className="text-[10px] text-slate-300 block mt-0.5">{block.sub}</span>
                    )}
                  </div>
                ))}
              </div>
