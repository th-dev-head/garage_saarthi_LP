...
const caHeroBlocks = [
  { label: "DEDICATED ACCOUNTANT ACCESS", title: "ca.sharma@auditfirm.com", titleClass: "text-xs font-bold text-amber-400 block" },
  { label: "READ-ONLY PERMISSION STATUS", title: "Read-Only • Financial Ledger Active", titleClass: "text-xs font-bold text-emerald-400 block" },
  { label: "TAX FILE EXPORT READY", title: "GSTR-1 & GSTR-3B Excel Synced", titleClass: "text-xs text-blue-400 font-semibold block" }
];

export default function CAHero() {
  const handleTrialClick = () => {
    trackEvent("ca_access_feature_trial_cta_click", {
      page_path: "/features/ca-access",
      cta_location: "hero",
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
          ...
          <div className="space-y-6">
            ...
            <p className="text-slate-300 text-sm md:text-base max-w-xl leading-relaxed">
              Grant your Chartered Accountant or tax consultant secure, read-only login access to your garage financial reports, GSTR filing data, expense ledgers, and P&L statements without sharing master admin passwords.
            </p>
            ...
            <div className="bg-white/10 backdrop-blur-md border border-white/10 rounded-2xl p-6 shadow-2xl w-full max-w-[340px] lg:max-w-md">
              <div className="flex justify-between items-center pb-4 mb-4 border-b border-white/10">
                <span className="text-xs font-bold text-white flex items-center gap-1.5">
                  <FaUserShield className="text-amber-400 w-4 h-4" /> CA Portal Security
                </span>
                <span className="text-[10px] text-emerald-400 bg-emerald-500/10 px-2 py-0.5 rounded font-semibold">Read-Only Secure</span>
              </div>
              <div className="space-y-4">
                {caHeroBlocks.map((block, idx) => (
