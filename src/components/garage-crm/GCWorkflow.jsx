    ...
    <section className="py-20 px-4 lg:px-15 2xl:px-50 bg-white flex justify-center border-b border-slate-200/60">
      <div className="mx-auto max-w-full lg:max-w-6xl w-full">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 lg:grid-cols-7 gap-4">
          {steps.map((s, idx) => (
            <div key={idx} className="bg-slate-50 border border-slate-200/80 p-5 rounded-2xl relative shadow-sm">
              <span className="absolute top-3 right-3 text-lg font-black text-slate-200">{s.num}</span>
              <h3 className="text-xs font-bold text-text-dark mb-2 pr-6">{s.title}</h3>
